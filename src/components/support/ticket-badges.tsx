...
export function TicketStatusBadge({ status }: { status: TicketStatus }) {
  const t = useTranslations("dashboard.pages.support.statuses");
  return <BadgeBase className={statusClasses[status]}>{t(status)}</BadgeBase>;
}
...
export function TicketPriorityBadge({
  ...
  const t = useTranslations("dashboard.pages.support.priorities");
  return (
    <BadgeBase className={priorityClasses[priority]}>{t(priority)}</BadgeBase>
  );
}
